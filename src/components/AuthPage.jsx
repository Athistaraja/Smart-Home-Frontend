  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `https://smart-home-backend-ubmj.onrender.com${endpoint}`,
        data,
        {
    ...
    >
      ...
      <div className="relative flex flex-col md:flex-row w-full max-w-4xl bg-white shadow-lg rounded-lg overflow-hidden">
        ...
        <motion.div
          ...
          transition={{ duration: 0.5 }}
          className={`w-full md:w-1/2 flex flex-col items-center justify-center p-10 transition-all ${
            isRegister ? "bg-green-500 text-white" : "bg-blue-500 text-white"
          }`}
        >
          <h2 className="text-3xl font-bold mb-4">
            {isRegister ? "Welcome Back!" : "Join Us!"}
          </h2>
          <p className="mb-6">
            {isRegister
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            <button
              disabled={loading}
              className={`bg-green-500 text-white px-6 py-3 rounded-full mt-6 w-full font-bold shadow-md transition duration-300 ${
